...
const CalendarPage = ({ user, token }) => {
    ...
    Promise.all([API.listTasks(token), API.listCourses(token)])
      .then(([tasksResp, coursesResp]) => {
        ...
        setTasks(tasksResp.map(t => ({
          ...
          color: (coursesResp.find(c => c.id === t.course_id)?.color) || 'blue',
        })));
      })
      .catch(console.error);
  }, [user, token]);
  ...
  const getTasksForDay = (day) => {
    ...
    return tasks.filter(task => (new Date(task.date)).toISOString().slice(0,10) === dateStr);
  };
  ...
  const getTasksForWeekDay = (offset) => {
    const dayDate = new Date(startOfWeek);
    dayDate.setDate(startOfWeek.getDate() + offset);
    const dateStr = dayDate.toISOString().slice(0,10);
    return tasks.filter(t => (new Date(t.date)).toISOString().slice(0,10) === dateStr);
  };
              ...
              {view === 'month' ? (
                ...
                <>
                  ...
                  <div className="grid grid-cols-7 gap-4">
                    {dayNames.map((_, idx) => (
                      <div key={idx} className="bg-white p-4 rounded-lg border min-h-[180px]">
                        <div className="text-sm font-medium text-gray-900 mb-2">
                          {new Date(startOfWeek.getFullYear(), startOfWeek.getMonth(), startOfWeek.getDate() + idx).toLocaleDateString()}
                        </div>
                        <div className="space-y-2">
                          {getTasksForWeekDay(idx).map(task => (
                            <div key={task.id} className={`text-xs p-2 rounded bg-${task.color}-100 text-${task.color}-800`}>{task.title}</div>
                          ))}
                        </div>
                      </div>
                    ))}
